...
def _backup_print(level, msg, *args, **kwargs):
    ...
    print("[{0} {1:5}] -> {2}   <------ [from {3}:{4}({5})]"
          .format(cur_time, _levelToName.get(level, "debug"), msg, rel_name, back_file.f_code.co_name,
                  back_file.f_lineno))
...
class ObLog(object):
    ...
    def debug(self, msg, *args, **kwargs):
        if self.level <= DEBUG:
            if self.log_type != "log":
                ...
                _backup_print(DEBUG, msg, *args, **kwargs)
    ...
    def info(self, msg, *args, **kwargs):
        if self.level <= INFO:
            if self.log_type != "log":
                args = tuple(map(str, args))
                _backup_print(INFO, msg, *args, **kwargs)
    ...
    def warning(self, msg, *args, **kwargs):
        if self.level <= WARNING:
            if self.log_type != "log":
                args = tuple(map(str, args))
                _backup_print(WARNING, msg, *args, **kwargs)
    ...
    def warn(self, msg, *args, **kwargs):
        if self.level <= WARN:
            if self.log_type != "log":
                args = tuple(map(str, args))
                _backup_print(WARN, msg, *args, **kwargs)
    ...
    def error(self, msg, *args, **kwargs):
        if self.level <= ERROR:
            if self.log_type != "log":
                args = tuple(map(str, args))
                _backup_print(ERROR, msg, *args, **kwargs)
    ...
    def critical(self, msg, *args, **kwargs):
        if self.level <= CRITICAL:
            if self.log_type != "log":
                args = tuple(map(str, args))
                _backup_print(CRITICAL, msg, *args, **kwargs)
    ...
    def fatal(self, msg, *args, **kwargs):
        if self.level <= FATAL:
            if self.log_type != "log":
                args = tuple(map(str, args))
                _backup_print(FATAL, msg, *args, **kwargs)
